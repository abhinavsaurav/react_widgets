...
const Dropdown = ({label, options, selected, onSelectedChange})=>{                 // can be props.options
    ...
    const renderedOptions= options.map((option)=>{
        if (option === selected){       // just to make sure the currently selected item doesn't shows up in the list
            return null;
        }
        return (
    ...
    return (            // using ternary operator to set the value of active or not based on the value of open state
